...
Creates a fully editable V(x, y)^2 surface calculated on the physical z=0 plane.
...
# Potential calculation: same electrostatic convention as InputChargeCalculate(2).py
FIELD_MODE = 'VSQ'               # This file visualizes the squared potential V^2
VERTICAL_AXIS_LABEL = "V^2"
EPSILON_0 = 1.0
POTENTIAL_GRID_SIZE = 32
SURFACE_DISPLAY_HEIGHT = 2.25    # Visual height; actual V^2 values stay in tick labels
SURFACE_COLOR_BANDS = 11         # Stable GLB material bands instead of complex shaders
...
def field_axis_suffix():
    return "V" if FIELD_MODE == 'V' else "V2"


def format_field_value(value):
    """Compact numeric labels that remain readable after Text -> Mesh."""
    if abs(value) < 5e-12:
        return "0"
    return f"{value:.3g}"


def create_field_axis(field_stats, axis_material, text_material,
                      axis_collection, tick_collection, label_collection):
    """Create the real mesh vertical axis representing V or V^2."""
    suffix = field_axis_suffix()
    top = SURFACE_DISPLAY_HEIGHT * 1.10
    bottom = -top if FIELD_MODE == 'V' else 0.0

    create_cylinder_between(
        name=f"Axis_{suffix}",
        point_a=(0.0, 0.0, bottom),
        point_b=(0.0, 0.0, top),
        radius=AXIS_RADIUS,
        material=axis_material,
        collection=axis_collection,
        vertices=AXIS_VERTICES,
    )

    arrow_center = Vector((0.0, 0.0, top + ARROW_LENGTH * 0.5))
    bpy.ops.mesh.primitive_cone_add(
        vertices=max(8, int(AXIS_VERTICES)),
        radius1=ARROW_RADIUS,
        radius2=0.0,
        depth=ARROW_LENGTH,
        end_fill_type='NGON',
        location=arrow_center,
    )
    arrow = bpy.context.object
    arrow.name = f"Arrow_{suffix}"
    arrow.data.name = f"Arrow_{suffix}_Mesh"
    assign_material(arrow, axis_material)
    set_cylinder_side_smooth(arrow)
    move_to_collection(arrow, axis_collection)

    fractions = (-1.0, -0.5, 0.0, 0.5, 1.0) if FIELD_MODE == 'V' \
        else (0.0, 0.25, 0.5, 0.75, 1.0)
    real_scale = field_stats["normalization_max"]

    for index, fraction in enumerate(fractions):
        z_display = fraction * SURFACE_DISPLAY_HEIGHT
        real_value = fraction * real_scale
        center = Vector((0.0, 0.0, z_display))
        half_tick = Vector((TICK_LENGTH * 0.65, 0.0, 0.0))
        create_cylinder_between(
            name=f"TickMark_{suffix}_{index:02d}",
            point_a=center - half_tick,
            point_b=center + half_tick,
            radius=TICK_RADIUS,
            material=axis_material,
        ...
        create_text_mesh(
            body=format_field_value(real_value),
            name=f"TickLabel_{suffix}_{index:02d}",
            location=(-TICK_LABEL_OFFSET * 2.0, 0.10, z_display),
            size=TICK_LABEL_SIZE * 0.90,
            material=text_material,
            collection=label_collection,
        )

    create_text_mesh(
        body=VERTICAL_AXIS_LABEL,
        name=f"Label_{suffix}",
        location=(0.0, 0.0, top + ARROW_LENGTH + AXIS_LABEL_OFFSET),
        size=AXIS_LABEL_SIZE,
        material=axis_material,
        collection=label_collection,
    )
...
# =============================================================================
# POTENTIAL SURFACE: z=0 OBSERVATION DOMAIN -> V OR V^2
# =============================================================================

def calculate_potential(x, y):
    """
    Calculate V(x,y,z=0) exactly as InputChargeCalculate(2).py:
    sum(q / (4*pi*epsilon_0*r)), with full 3D source distance.
    """
    total = 0.0
    factor = 1.0 / (4.0 * math.pi * EPSILON_0)
    for charge in CHARGES:
        qx, qy, qz = (float(value) for value in charge["pos"])
        distance = math.sqrt((x - qx) ** 2 + (y - qy) ** 2 + qz ** 2)
        if distance <= 1e-12:
            raise ValueError("A charge lies exactly on a sampled observation point")
        total += factor * float(charge["q"]) / distance
    return total


def calculate_potential_field():
    count = max(2, int(POTENTIAL_GRID_SIZE))
    half_size = float(PLANE_HALF_SIZE)
    coordinates = [
        -half_size + (2.0 * half_size * index / (count - 1))
    ...
    for y in coordinates:
        for x in coordinates:
            value = calculate_potential(x, y)
            signed_potential.append(value)
            field_values.append(value if FIELD_MODE == 'V' else value * value)

    field_min = min(field_values)
    field_max = max(field_values)
    if FIELD_MODE == 'V':
        normalization_max = max(abs(field_min), abs(field_max))
    else:
        normalization_max = field_max
    if normalization_max <= 1e-15:
        raise ValueError("Calculated potential field is numerically zero")

    heights = [
        value / normalization_max * SURFACE_DISPLAY_HEIGHT
    ...
    stats = {
        ...
        "field_min": field_min,
        "field_max": field_max,
        "potential_min": min(signed_potential),
        "potential_max": max(signed_potential),
        "normalization_max": normalization_max,
        "display_units_per_field_unit": SURFACE_DISPLAY_HEIGHT / normalization_max,
    }
    return coordinates, field_values, heights, stats


def interpolate_color(color_a, color_b, amount):
    amount = max(0.0, min(1.0, float(amount)))
    return tuple(
        color_a[index] + (color_b[index] - color_a[index]) * amount
        for index in range(3)
    ) + (1.0,)


def potential_band_color(amount):
    """Diverging colors for V; sequential colors for nonnegative V^2."""
    amount = max(0.0, min(1.0, float(amount)))
    if FIELD_MODE == 'V':
        negative = (0.055, 0.20, 0.82)
        neutral = (0.92, 0.93, 0.95)
        positive = (0.92, 0.10, 0.055)
        if amount <= 0.5:
            return interpolate_color(negative, neutral, amount * 2.0)
        return interpolate_color(neutral, positive, (amount - 0.5) * 2.0)

    low = (0.10, 0.07, 0.30)
    middle = (0.10, 0.55, 0.78)
    high = (1.00, 0.43, 0.035)
    if amount <= 0.5:
        return interpolate_color(low, middle, amount * 2.0)
    return interpolate_color(middle, high, (amount - 0.5) * 2.0)


def create_potential_surface(collection):
    coordinates, values, heights, stats = calculate_potential_field()
    count = stats["grid_size"]
    suffix = field_axis_suffix()
    object_name = f"Potential_Surface_{suffix}"

    vertices = []
    for y_index, y in enumerate(coordinates):
        for x_index, x in enumerate(coordinates):
            flat_index = y_index * count + x_index
            vertices.append((x, y, heights[flat_index]))

    faces = []
    face_values = []
    for y_index in range(count - 1):
        for x_index in range(count - 1):
            a = y_index * count + x_index
            b = a + 1
            d = (y_index + 1) * count + x_index
            c = d + 1
            faces.append((a, b, c, d))
            face_values.append((values[a] + values[b] + values[c] + values[d]) * 0.25)
    ...
    band_count = max(3, int(SURFACE_COLOR_BANDS))
    for band in range(band_count):
        amount = band / (band_count - 1)
        material = create_material(
            f"Potential_{suffix}_Band_{band:02d}",
            potential_band_color(amount),
            roughness=0.46,
        )
        obj.data.materials.append(material)

    normalization_max = stats["normalization_max"]
    for polygon, face_value in zip(obj.data.polygons, face_values):
        if FIELD_MODE == 'V':
            normalized = 0.5 + 0.5 * face_value / normalization_max
        else:
            normalized = face_value / normalization_max
        band = int(round(max(0.0, min(1.0, normalized)) * (band_count - 1)))
        polygon.material_index = band
        polygon.use_smooth = True

    obj["field_mode"] = FIELD_MODE
    obj["formula"] = "sum(q/(4*pi*epsilon_0*r))" + ("^2" if FIELD_MODE == 'VSQ' else "")
    obj["observation_plane_physical_z"] = 0.0
    obj["epsilon_0"] = float(EPSILON_0)
    obj["grid_size"] = int(count)
    obj["actual_field_min"] = float(stats["field_min"])
    obj["actual_field_max"] = float(stats["field_max"])
    obj["display_units_per_field_unit"] = float(stats["display_units_per_field_unit"])

    print(
        f"[Potential Surface] mode={FIELD_MODE} grid={count}x{count} "
        f"V=[{stats['potential_min']:.8g}, {stats['potential_max']:.8g}] "
        f"field=[{stats['field_min']:.8g}, {stats['field_max']:.8g}]"
    )
    return obj, stats
...
def validate_result():
    ...
    suffix = field_axis_suffix()
    required_meshes = {
        "Axis_X", "Axis_Y", f"Axis_{suffix}",
        "Arrow_X", "Arrow_Y", f"Arrow_{suffix}",
        "Label_X", "Label_Y", f"Label_{suffix}",
        "Origin", "Observation_Plane", "Grid",
        f"Potential_Surface_{suffix}",
    }
    missing = [
    ...
    unexpected_sources = [
        ...
        if obj.name.startswith(("Charge_", "ChargeSymbol_", "Projection_"))
    ]
    if unexpected_sources:
        raise RuntimeError(f"Source-space objects leaked into field graph: {unexpected_sources}")

    mesh_count = sum(1 for obj in bpy.context.scene.objects if obj.type == 'MESH')
    print(f"[Potential Model] Validation passed: {mesh_count} mesh objects, "
...
def main():
    ...
    potential_field = create_collection("Potential_Field")
    ...
    _, field_stats = create_potential_surface(potential_field)
    create_field_axis(
        field_stats=field_stats,
        axis_material=materials["AxisZ"],
        text_material=materials["Text"],
        axis_collection=coordinate_system,
        tick_collection=tick_marks,
        label_collection=labels,
    )
    ...
    bpy.ops.object.select_all(action='DESELECT')
    print(f"[Potential Model] {FIELD_MODE} scene generation complete.")
